...
def assign_row_sequences(txs: list[Transaction]) -> list[Transaction]:
    ...
    Transactions are grouped by (account_label, posted_date, amount); within
    each group, sequences are assigned 0, 1, 2, ... in the order the
    transactions appear in ``txs``. This disambiguates duplicate lines (e.g.
    two identical $5 coffee charges on the same day on the same card).
    """
    counters: dict[tuple[str, str, Decimal], int] = defaultdict(int)
